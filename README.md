...
| 上游提供 channels/manifest.json 发布清单(如 qoderwake-cn、qoder-cli-cn) | `strategy :json` + 指向 manifest 的 `url`,block 里 `json["latest"]`。manifest 里每个平台的 url/sha256 都有(已抽验与实测下载一致),手写 bump 时可直接取用 |
